...
I want to add some sort of formatting visual display for our json fields.

You have full liberty to design and implement the best system to cover these goals:

1. Each json field is rendered distinctly and cleanly
2. Clicking inside a json field copies the prompt text to clipboard
3. Plays nicely with our display filtering of fields (or we can modify this also)

In addition to the task above, I want to create a more robust json helper. This will try to capture badly formatted json outputs from lesser intelligent agents who screw up and fail to output valid json.

If there's not a good spot for either of these in the code, consider creating a new file(s).

### PROJECT 

A sophisticated desktop application that enhances image generation prompts using AI models, and in the future, will provide seamless integration with various image generation APIs, using a modern PyQt6 GUI.
...
1. `gui/widgets/response_panel.py`
2. `app/controller.py`
